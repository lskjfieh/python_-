...
class Face_Register:
    def __init__(self):
        ...
        self.current_frame_faces_cnt = 0  # 当前帧中人脸计数器
        self.existing_faces_cnt = 0  # 已录入的人脸计数器
        self.ss_cnt = 0  # 录入 person_n 人脸时图片计数器 
        ...
        self.label_face_cnt = tk.Label(self.frame_right_info, text="当前页面人脸数: ")
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 text="数据库中当前人脸: ").grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="当前页面人脸: ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="第一步：设置姓名").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="第二步：保存当前人脸").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "请不要离开摄像范围!"
            else:
                self.log_all["text"] = "当前页面未识别到人脸!"
        else:
            self.log_all["text"] = "请先运行第一步!"
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    if (d.right() + self.ww) > 640 or (d.bottom() + self.hh > 480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
                        self.label_warning["text"] = "超出摄像范围！"
